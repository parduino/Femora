...
The ``MeshPartManager`` is a core component of the MeshMaker library that serves as a centralized system for creating, retrieving, tracking, and managing mesh part objects throughout the mesh generation process. Similar to the MaterialManager and ElementRegistry, it implements the Singleton pattern to ensure a single, consistent point of mesh part management across the entire application.

Mesh parts defined in MeshMaker are automatically tracked, categorized, and organized by the MeshPartManager, simplifying the process of creating complex models with multiple mesh geometries.
...
2. **Through MeshMaker** (Recommended): Access via the MeshMaker class's .meshPart property
...
The mesh part types available in MeshMaker are organized by their geometric dimensionality. Follow the links above to explore the various mesh part types available in each category.
