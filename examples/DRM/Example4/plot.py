...
if __name__ == "__main__":
    ...
    boundaryConditionType = "SW4"
    boundaryConditionType = "Results"
    ...
    max_time = 20 # read full time series up to this time
    res_type = "acceleration"

    # Read data for all observation points
    data = []
    for i, op in enumerate(ops):
        if boundaryConditionType == "SW4":
            ax = np.loadtxt(f"./SW4/P{i}/A_x.txt")
            ay = np.loadtxt(f"./SW4/P{i}/A_y.txt")
            az = np.loadtxt(f"./SW4/P{i}/A_z.txt")
            dt = 1.077828176e-02
            t = np.arange(0, ax.shape[0] * dt, dt)
            a = np.zeros((ax.shape[0], 3))
            mask=t < max_time
            t = t[mask]
            ax = ax[mask]
            ay = ay[mask]
    ...
    if not data:
        ...
        fig, ax = plt.subplots(nrows, 3, figsize=(9.5, fig_height), dpi=300, sharex=True, sharey="row")
