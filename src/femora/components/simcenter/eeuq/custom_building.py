...
# Input parameters
# =============================================================================
# soil Profile is from bottom to top
structure_info = {
    "num_partitions": 1,
    "x_min": 0.,
    "y_min": 0.,
...
for layer_index, layer in enumerate(soil_profile_info["soil_profile"]):
...
Z_MIN = min([layer["z_bot"] for layer in soil_profile_info["soil_profile"]])
Z_MAX = max([layer["z_top"] for layer in soil_profile_info["soil_profile"]])
# print("Z_MIN = ", Z_MIN)
# print("Z_MAX = ", Z_MAX)

if soil_profile_info["nx"] < 1 or soil_profile_info["ny"] < 1:
    print("Error: Number of cells in X or Y direction is less than 1")
    sys.exit(1)


# check the piles
for pile_index, pile in enumerate(pile_profile_info["pile_profile"]):
    ...
    x_min = soil_profile_info["x_min"]
    x_max = soil_profile_info["x_max"]
    y_min = soil_profile_info["y_min"]
    y_max = soil_profile_info["y_max"]
...
if structure_info["x_min"] < soil_profile_info["x_min"] or \
   structure_info["x_max"] > soil_profile_info["x_max"] or \
   structure_info["y_min"] < soil_profile_info["y_min"] or \
   structure_info["y_max"] > soil_profile_info["y_max"] or \
...
for foundation_index, foundation in enumerate(foundation_mesh):
    ...
    for pile in pile_profile_info["pile_profile"]:
...
for layer_index, layer in enumerate(soil_profile_info["soil_profile"]):
    ...
    nx = soil_profile_info["nx"]
    ny = soil_profile_info["ny"]    
    nz = layer["nz"]
...
for layer_index, layer in enumerate(soil_profile_info["soil_profile"]):
    ...
    elem = fm.element.brick.std(ndof=3, 
                                material=mat,
                                b1=soil_profile_info["gravity_x"]*rho,
                                b2=soil_profile_info["gravity_y"]*rho,
                                b3=soil_profile_info["gravity_z"]*rho)
...
for pile_index, pile in enumerate(pile_profile_info["pile_profile"]):
    radius = pile["r"]
    n_p = pile_profile_info["pile_interface"]["num_points_on_perimeter"]
    n_l = pile_profile_info["pile_interface"]["num_points_along_length"]
    pp = pile_profile_info["pile_interface"]["penalty_parameter"]
...
fm.assembler.create_section(soil_sections, 
                            num_partitions=soil_profile_info["num_partitions"],
                            partition_algorithm="kd-tree",
                            merging_points=True,
                            tolerance=1e-3,
                            mass_merging="sum")
...
if soil_profile_info["boundary_conditions"] == "periodic": 
    z_min = np.inf
    z_max = -np.inf
    for layer in soil_profile_info["soil_profile"]:
    ...
    fm.constraint.sp.fixMacroZmin(dofs=[1,1,1],
                                tol=1e-3)
elif soil_profile_info["boundary_conditions"] == "DRM":

    fm.drm.addAbsorbingLayer(numLayers=soil_profile_info["DRM_options"].get("number_of_layers"),
                        numPartitions=soil_profile_info["num_partitions"],
                        partitionAlgo="kd-tree",
                        geometry="Rectangular",
                        rayleighDamping=soil_profile_info["DRM_options"].get("Rayleigh_damping"),
                        matchDamping=soil_profile_info["DRM_options"].get("match_damping", False),
                        type=soil_profile_info["DRM_options"].get("absorbing_layer_type", "Rayleigh")
                        )
