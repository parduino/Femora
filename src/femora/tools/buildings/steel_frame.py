...
class FEMA_SAC_SteelFrame(Building):
    """
    ...
    def create_rigid_diaphragms(self, model, verbose: bool = True):
        ...
        for s in range(1, self.num_stories + 1):
            ...
            # Keep diaphragm master nodes vertical/rotationally stable while
            # leaving in-plane translation and Rz free for lateral modes.
    ...
    def get_modes(
        ...
        *,
        ...
        Raises:
            ...
            RuntimeError: If OpenSees is not configured, fails to run, or does
                not return the expected modal data.
        ...
        cloned_frame = FEMA_SAC_SteelFrame(
            ...
            length_unit_system=self.section_unit_system,
            origin=copy.deepcopy(self.origin),
            floor_masses=copy.deepcopy(self.floor_masses),
        )
        cloned_frame.col_sections = copy.deepcopy(self.col_sections)
        cloned_frame.beam_x_sections = copy.deepcopy(self.beam_x_sections)
        cloned_frame.beam_y_sections = copy.deepcopy(self.beam_y_sections)

        with tempfile.TemporaryDirectory(prefix="femora_eigen_") as tmpdir:
            ...
            try:
                with contextlib.redirect_stdout(setup_output), contextlib.redirect_stderr(setup_output):
                    ...
                    model.constraint.sp.fix_macro_z_min(dofs=[1, 1, 1, 1, 1, 1], tol=0.01)
...
"""
            ...
            if print_results:
                self._print_modal_table(
                    result["frequencies"],
                    result["periods"],
                    title="FEMA SAC Steel Frame Modal Results",
                )
            if plot:
